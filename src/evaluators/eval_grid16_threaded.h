...
void workload_grid16( workload_grid16_parms_t const * const parms )
{
	...
	const __m512 p0_x = _mm512_set1_ps( parms->p0.x );
	const __m512 p0_y = _mm512_set1_ps( parms->p0.y );
	const __m512 p0_z = _mm512_set1_ps( parms->p0.z );
	const __m512 stepsiz_x = _mm512_set1_ps( parms->stepsiz.x );
	const __m512 stepsiz_y = _mm512_set1_ps( parms->stepsiz.y );
	const __m512 stepsiz_z = _mm512_set1_ps( parms->stepsiz.z );

	const __m512 D_MAX = _mm512_set1_ps( FLT_MAX );

	__m512i ofs;
	for ( int i=0,n=SIMD_SIZ; i<n; ++i )
		ofs.m512i_i32[i] = parms->minidx + i;
	__m512i ii = ofs;

	#if ( USE_PROFILER == PROFILER_SUPERLUMINAL )
	char profiler_buf[512];
	#endif
	...
	for ( int32_t idx_block=block_startidx, nidx_block=block_startidx+block_count; idx_block<nidx_block; ++idx_block )
	{
		...
		for ( int i=0,n=SIMD_SIZ; i<n; ++i )
		{
			...
			int32_t ix =  ii.m512i_i32[i] % (dim_x);
			int32_t iy = (ii.m512i_i32[i] / dim_x) % dim_y;
			int32_t iz =  ii.m512i_i32[i] / (dim_x*dim_y);
		
			idx_x.m512_f32[i] = (float32_t)ix;
			idx_y.m512_f32[i] = (float32_t)iy;
			idx_z.m512_f32[i] = (float32_t)iz;
		}
		//__m512 ii = _mm512_fmadd_ps( 

		const __m512 p_x = _mm512_fmadd_ps( stepsiz_x, idx_x, p0_x );
		const __m512 p_y = _mm512_fmadd_ps( stepsiz_y, idx_y, p0_y );
		const __m512 p_z = _mm512_fmadd_ps( stepsiz_z, idx_z, p0_z );

		//#ifndef NDEBUG
		//parms->sdf->eval_points[SIMD_SIZ*idx_block + i] = vec3_t( p_x.m512_f32[i], p_y.m512_f32[i], p_z.m512_f32[i] );
		//#endif //NDEBUG

		#if ( USE_PROFILER == PROFILER_SUPERLUMINAL )
		sprintf_s(profiler_buf, 512, "block=%d", idx_block);
		...
		ii = _mm512_add_epi32( ii, _mm512_set1_epi32(SIMD_SIZ) );
...
void eval_sdf__grid16_threaded( sdf_t &sdf, lpt::indexed_triangle_mesh_t const * const mesh )
{
	...
	if ( num_cells_remaining > 0 )
	{
		...
		__m512 p_z = _mm512_set1_ps(0.0f);
		...
		for ( int i=0,n=num_cells_remaining; i<n; ++i )
		{
			...
			p_x.m512_f32[i] = p0.x + stepsiz.x * (float32_t)ix;
			p_y.m512_f32[i] = p0.y + stepsiz.y * (float32_t)iy;
			p_z.m512_f32[i] = p0.z + stepsiz.z * (float32_t)iz;
		}
