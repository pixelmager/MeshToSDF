...
{
	uint32_t nToken = MicroProfileTimelineEnterInternal(nColor, pStr, (int)strlen(pStr), true);
	(void)nToken;	
}
...
uint32_t MicroProfileTimelineEnterf(uint32_t nColor, const char* pStr, ...)
{
	...
	buffer[size] = '\0';
	return MicroProfileTimelineEnterInternal(nColor, buffer, (int)size, false);
}
