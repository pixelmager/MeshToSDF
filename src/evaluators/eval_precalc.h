...
float32_t udTriangle_sq_precalc( const vec3_t &p, const tri_precalc_t &pc )
{
	...
	return ((sign(dot(pc.cp0,p1)) + 
             ...
             ?
             min( min( 
             dot2(v21*clamp(dot(v21,p1)*pc.rcp_dp2_v21_v32_v13.x,0.0f,1.0f)-p1),
             dot2(v32*clamp(dot(v32,p2)*pc.rcp_dp2_v21_v32_v13.y,0.0f,1.0f)-p2) ),
             dot2(v13*clamp(dot(v13,p3)*pc.rcp_dp2_v21_v32_v13.z,0.0f,1.0f)-p3) )
             :
