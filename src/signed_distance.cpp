...
int main()
{
	...
	sdf_t sdf;
	{
...

		{
			...
			//int i = cpuinfo.num_cores_logical;
		...
		sdf_t sdf_sse;
		{
			init_sdf(&sdf_sse, bb, GRID_SIZ_X, GRID_SIZ_Y, GRID_SIZ_Z, 4);
			...
			{
				const uint64_t t0 = gettime_ms();
				eval_sdf__grid4_threaded(sdf_sse, mesh, i);
				const uint64_t t1 = gettime_ms();
				printf("%d\n", (int)(t1 - t0));
			}
			printf( "scalar == sse: %s\n", equals_sdf( &sdf, &sdf_sse ) ? "true" : "false" );
			printf( "scalar ~= sse: %s\n", equals_approximately_sdf( &sdf, &sdf_sse ) ? "true" : "false" );
		...
		if ( path_avx256 )
        {
			...
			printf( "sse == avx256: %s\n", equals_sdf( &sdf_sse, &sdf) ? "true" : "false" );
			printf( "sse ~= avx256: %s\n", equals_approximately_sdf( &sdf_sse, &sdf) ? "true" : "false" );
